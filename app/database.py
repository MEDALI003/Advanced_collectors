...
async def ingest(
    ...
    upsert_agent(agent_data, client_ip, seen_at)
    insert_metrics(agent_data, client_ip, payload.metrics.model_dump(), seen_at)
    insert_logs(agent_data, client_ip, [x.model_dump() for x in payload.logs])
    insert_services(agent_data, client_ip, [x.model_dump() for x in payload.services])
    insert_file_events(agent_data, client_ip, [x.model_dump() for x in payload.file_events])
    insert_network_connections(
        agent_data,
        client_ip,
