...
def to_mysql_dt(value: str | None) -> str | None:
    if not value:
        return None
    try:
...
def upsert_agent(agent: dict, ip: str, ts: str) -> None:
    ts = to_mysql_dt(ts) or datetime.utcnow().strftime("%Y-%m-%d %H:%M:%S.%f")
...
def insert_metrics(agent: dict, ip: str, metrics: dict, ts: str) -> None:
    ...
    with get_conn() as conn:
        ...
        """, (
            agent["hostname"],
            ip,
            to_float(metrics.get("cpu")),
            to_float(metrics.get("memory")),
            to_float(metrics.get("disk")),
            to_float(metrics.get("load_1m")),
            str(metrics.get("boot_time")) if metrics.get("boot_time") is not None else None,
            ts,
        ))
        cur.close()
...
def insert_logs(agent: dict, ip: str, logs: list[dict]) -> None:
    ...
    for item in logs:
        ...
        timestamp = to_mysql_dt(item.get("timestamp"))
        ...
        rows.append((
            ...
            str(item.get("service")) if item.get("service") is not None else None,
            str(item.get("level")) if item.get("level") is not None else None,
            message,
            timestamp,
        ))
...
def insert_services(agent: dict, ip: str, services: list[dict]) -> None:
    ...
    for item in services:
        ...
        rows.append((
            ...
            ip,
            service_name,
            str(item.get("active_state")) if item.get("active_state") is not None else None,
            str(item.get("sub_state")) if item.get("sub_state") is not None else None,
            timestamp,
        ))
...
def insert_file_events(agent: dict, ip: str, events: list[dict]) -> None:
    ...
    for item in events:
        ...
        timestamp = to_mysql_dt(item.get("timestamp"))
        if not path or not action or not timestamp:
            continue

        rows.append((agent["hostname"], ip, path, action, timestamp))
...
def insert_network_connections(agent: dict, ip: str, items: list[dict]) -> None:
    ...
    for item in items:
        ...
        rows.append((
            agent["hostname"],
            ip,
            str(item.get("protocol")) if item.get("protocol") is not None else None,
            str(item.get("local_address")) if item.get("local_address") is not None else None,
            str(item.get("remote_address")) if item.get("remote_address") is not None else None,
            str(item.get("status")) if item.get("status") is not None else None,
            to_int(item.get("pid")),
            str(item.get("process_name")) if item.get("process_name") is not None else None,
            timestamp,
        ))
...
def insert_top_processes(agent: dict, ip: str, items: list[dict]) -> None:
    ...
    for item in items:
        ...
        rows.append((
            ...
            str(item.get("username")) if item.get("username") is not None else None,
            to_float(item.get("cpu_percent")),
            to_float(item.get("memory_percent")),
            timestamp,
        ))
